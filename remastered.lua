if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Moon Yield Remastered is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Moon Yield Remastered is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end

currentVersion = "6.8.9"
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Moon Yield Remastered Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://lycheefeather.github.io/moon-yield/remastered.lua'))()")
	end
end)
...
addcmd('generatenonce',{}, function(args, speaker)
  local nonce = HttpService:GenerateGUID(true)
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Moon Yield Remastered may have issues or features that are not functioning correctly.") end
end)
