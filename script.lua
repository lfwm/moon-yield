...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'Moon Client is waiting for the game to load'
...
Logo.Image = "rbxassetid://18872504686"
...
CMDs[#CMDs + 1] = {NAME = 'destroygui', DESC = 'Destroys GUI'}
...
CMDs[#CMDs + 1] = {NAME = 'toggleantibang', DESC = 'Toggles antibang'}
CMDs[#CMDs + 1] = {NAME = 'abanimcheck', DESC = 'Checks for any possible animations used in bang for the antibang command before triggering the prevention'}
		...
		task.spawn(function()
        	...
        	if weaoBody then
				local function addweaotemplate()
					for i,v in ipairs(weaoBody[1]) do
						local frame1 = Instance.new("Frame")
						local textlabel = Instance.new("TextLabel")
						local textlabel2 = Instance.new("TextLabel")
						local textlabel3 = Instance.new("TextLabel")
...
addcmd('invisible',{'invis'},function(args, speaker)
	...
	Character:MoveTo(Vector3.new(9e9,10,9e9))
	task.wait()
	Character:FindFirstChild("HumanoidRootPart").CFrame = CFrame.new(9e9,-800,9e9)
...
loadstring(game:HttpGetAsync("https://pastebin.com/raw/8DTCC8cK"))()
end)
...
addcmd("vulnspatch",{'vpatcher','vulnspatcher'},function(args, speaker)
	loadstring(game:HttpGet("https://infyieldreborn.github.io/vulnpatcher"))()
...
addcmd("antibang",{},function(args, speaker)
	...
	notify("Antibang","Anti-bang has been started.")
end)

addcmd("unantibang",{},function(args, speaker)
	AntiBangEnabled = false
	notify("AntiBang","Anti-bang has been stopped.")
end)

addcmd("toggleantibang",{},function(args, speaker)
	AntiBangEnabled = not AntiBangEnabled
	if (AntiBangEnabled) then notify("Antibang","Anti-bang has been started.") else notify("Antibang","Anti-bang has been stopped.") end
end)

addcmd("abanimcheck", {}, function(args, speaker)
	AnimCheck = not AnimCheck

	if (AnimCheck) then notify("AntiBang", "Animation checker has been enabled.") else notify("AntiBang", "Animation checker has been disabled.") end
end)

addcmd("mantibang", {}, function(args, speaker)
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then
		notify("Moon Client has loaded. Made by pkplaysrblx and LycheeFeather And credits to FB_Real!")
		if game.PlaceId == 574746640 then
			task.wait(3)
